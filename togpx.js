...
function togpx( geojson, options ) {
  ...
  features.forEach(function mapFeature(f) {
    switch (f.geometry.geometry.type) {
    // POIs
    case "Point":
    case "MultiPoint":
      var coords = f.geometry.geometry.coordinates;
      if (f.geometry.geometry.type == "Point") coords = [coords];
    ...
    case "MultiLineString":
      ...
      if (f.geometry.geometry.type == "LineString") coords = [coords];
      o = {
        "name": options.featureTitle(f.properties),
        "desc": options.featureDescription(f.properties)
      };
    ...
    case "MultiPolygon":
      ...
      var coords = f.geometry.geometry.coordinates;
      var times = options.featureCoordTimes(f);
      if (f.geometry.geometry.type == "Polygon") coords = [coords];
    ...
    case "GeometryCollection":
      f.geometry.geometry.geometries.forEach(function (geometry) {
        var pseudo_feature = {
      ...
      break;
    default:
      console.log("warning: unsupported geometry type: "+f.geometry.geometry.type);
    }
  });
